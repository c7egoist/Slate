...
namespace
{
    ...
    SpatialDirection Added(const SpatialDirection& LeftDirection, const SpatialDirection& RightDirection)
    {
    ...
    bool ResolveCurveTangent(const SketchStructure& Declared,
                             ...
                             CurveCutPosition& Cut)
    {
        ...
        if (!ResolveCurveCutPosition(Geometry, AtStart ? SpatialPoint{} : SpatialPoint{}, Cut))
        {
            // dummy initialization below replaces these for supported curves
        }
        ...
        switch (Geometry.Subject())
        {
            case CurveSubject::Line:
            {
                Point = Use.SameSense ? (AtStart ? Geometry.HeldLine().Origin : Geometry.HeldLine().Terminus)
                                      : (AtStart ? Geometry.HeldLine().Terminus : Geometry.HeldLine().Origin);
                Tangent = Normalize(Use.SameSense ? Difference(Geometry.HeldLine().Origin, Geometry.HeldLine().Terminus)
                                                  : Difference(Geometry.HeldLine().Terminus, Geometry.HeldLine().Origin));
                Cut = { AtStart ? 0.0 : 1.0, Point };
                return true;
            }
            case CurveSubject::CircularArc:
            {
                const CircularArcCurve& Arc = Geometry.HeldCircularArc();
                const SpatialDirection StartDirection = Normalize(Arc.StartDirection);
                const double Parameter = AtStart ? 0.0 : 1.0;
                const double Radians = Arc.SweepRadians * Parameter;
                const SpatialDirection Radial = Normalize(RotateAroundAxis(StartDirection, Arc.Normal, Radians));
                Point = Added(Arc.Centre, Scaled(Radial, Arc.Radius));
                Tangent = Normalize(Cross(Arc.Normal, Radial));
                if (!Use.SameSense)
                    Tangent = Scaled(Tangent, -1.0);
                Cut = { Parameter, Point };
                return true;
            }
            case CurveSubject::EllipticalArc:
            {
                CurveCutPosition Local = {};
                if (!ResolveCurveCutPosition(Geometry, AtStart ? SpatialPoint{} : SpatialPoint{}, Local)) {}
                const std::vector<SpatialPoint> Polyline = [&]() { std::vector<SpatialPoint> P; AppendCurvePolyline(Geometry, P, 96u); return P; }();
                if (Polyline.size() < 2u)
                    return false;
                Point = AtStart ? Polyline.front() : Polyline.back();
                const SpatialPoint Neighbor = AtStart ? Polyline[1] : Polyline[Polyline.size()-2u];
                Tangent = Normalize(AtStart ? Difference(Point, Neighbor) : Difference(Neighbor, Point));
                if (!Use.SameSense)
                    Tangent = Scaled(Tangent, -1.0);
                Cut = { AtStart ? 0.0 : 1.0, Point };
                return true;
            ...
            case CurveSubject::Hermite:
            {
                ...
                if (!Use.SameSense)
                    Tangent = Scaled(Tangent, -1.0);
                Cut = { AtStart ? Geometry.Interval().Minimum : Geometry.Interval().Maximum, Point };
                return true;
            }
            case CurveSubject::SubjectCount:
                return false;
