//============================================================================================================================================
//                                                  WORLDDRAFTTRANSFORMSESSION.CPP
//============================================================================================================================================
...
SpatialDirection ResolveAxisDirection(const WorldDraftTransformSession& Session)
{
                                      ...
                                      std::vector<WorldPlacementSubject>& Placements)
{
    ...
    {
        if (!ResolveWorldDraftPoints(Declared, { CurveIndex }, Points))
            continue;
...
void CollectConnectedCurvePlacements(const WorldDraftStructure& Declared,
...
SpatialDirection ResolveWorldOffset(const WorldDraftTransformSession& Session,
                                     ...
                                     std::vector<WorldPlacementSubject>& Placements)
{
    ...
    {
        SpatialPoint Position = {};
        if (!ResolveWorldDraftPointPosition(Declared, Target.Point, Position))
            return false;
                                   ...
                                   const SpatialDirection& Offset)
{
    for (std::size_t Index = 0u; Index < Session.Placements.size() && Index < Session.Origins.size(); ++Index)
    {
        ...
        if (Placement.ControlPlacement)
            Discard(EnforceWorldDraftControl(Declared, Placement.Control, Position));
        else
            Discard(EnforceWorldDraftPoint(Declared, Placement.Point, Position));
    }
}
