...
int main()
{
    std::printf("[WorldDraftEditingProof] world-space transforms deform the same closed shape in true 3D\n");
    ...
    //----------------------------------------------------------------------------------------------------
    {
        ...
        const WorldLoopName Loop = Draft.DeclareLoop({ { { AB, true }, { BC, true }, { CD, true }, { DA, true } } });

        Require(!!MoveWorldDraftCurve(Draft, BC, { 20.0, 0.0, 0.0 }),
                "moving one selected edge must succeed");
        Require(Draft.CurveCount() == 4u,
                "and it must not create duplicate curves while moving that edge");

        const DeclaredWorldCurve* HeldAB = Draft.Resolve(AB);
        const DeclaredWorldCurve* HeldBC = Draft.Resolve(BC);
        const DeclaredWorldCurve* HeldCD = Draft.Resolve(CD);
        const DeclaredWorldCurve* HeldDA = Draft.Resolve(DA);
    ...
    //----------------------------------------------------------------------------------------------------
    {
        ...
        Require(ResolveWorldDraftPoints(Draft, BC, Points) && Points.size() == 2u,
                "the lifted corner must be one of the selected edge endpoints");
        Require(!!MoveWorldDraftPoint(Draft, Points[1u].Name, { 0.0, 0.0, 25.0 }),
    ...
    {
        WorldDraftStructure Draft;
        const WorldCurveName AB = Draft.DeclareLine({ -50.0, -50.0, 10.0 }, {  50.0, -50.0, 10.0 }, Front);
        const WorldCurveName BC = Draft.DeclareLine({  50.0, -50.0, 10.0 }, {  50.0,  50.0, 10.0 }, Front);
        const WorldCurveName CD = Draft.DeclareLine({  50.0,  50.0, 10.0 }, { -50.0,  50.0, 10.0 }, Front);
        const WorldCurveName DA = Draft.DeclareLine({ -50.0,  50.0, 10.0 }, { -50.0, -50.0, 10.0 }, Front);
        const WorldLoopName Loop = Draft.DeclareLoop({ { { AB, true }, { BC, true }, { CD, true }, { DA, true } } });

        WorldPick Pick = {};
        Pick.Subject = WorldPickSubject::Loop;
        Pick.Loop = Loop;
        Require(!!MoveWorldDraftPick(Draft, Pick, { 0.0, 0.0, 60.0 }),
    ...
    //----------------------------------------------------------------------------------------------------
    {
        ...
        Pick.Curve = Bezier;
        Require(!!MoveWorldDraftPick(Draft, Pick, { 0.0, 0.0, 30.0 }),
                "moving a picked Bezier control in Z must succeed");

        const DeclaredWorldCurve* Held = Draft.Resolve(Bezier);
    ...
    std::printf("[WorldDraftEditingProof] %u claims, %u failures\n", Claims, Failures);
